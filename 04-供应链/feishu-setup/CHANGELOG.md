...
## [2026-04-14] 批量ZIP导出 + Web化物流操作 + 专业标签

### 需求

助理痛点：(1) 每天11am需导出所有订单给工厂，不能逐单下载；(2) 物流操作需要命令行，助理不会用；(3) 标签设计太简陋。

### 交付物

| 模块 | 文件 | 说明 |
|------|------|------|
| 批量ZIP导出 | `server.js` | `GET /api/admin/batch-zip` — 多订单合并为一个ZIP，每单子目录含Excel+QR+标签 |
| Web物流操作 | `server.js` | `POST /api/admin/confirm` / `ship` / `deliver` — 批量确认/发货/签收 |
| 管理页升级 | `labels.html` | 新增确认订单/确认发货/标记签收按钮，导出ZIP改为批量合并下载 |
| 专业标签 | `server.js` | 80×50mm 标签升级：红/蓝眼别色带、处方参数网格、GAUSH|CLEAR品牌 |
| IP修复 | `logistics.js` | 硬编码IP改为 `SERVER_BASE_URL` 环境变量 |

### 助理操作流程（全部在浏览器完成）

1. 打开 `/labels?admin=xxx`
2. 勾选待确认订单 → 点「确认订单」
3. 勾选待发货订单 → 点「导出ZIP」→ 发给工厂
4. 工厂寄回后 → 点「确认发货」→ 自动生成快递单号
5. 选中订单 → 点「预览标签/打印」→ 打印专业标签
6. 签收后 → 点「标记签收」
