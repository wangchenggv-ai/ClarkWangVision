# 订单交付系统 — 架构说明

**最后更新：** 2026-04-14

---

## 一、E2E 全链路

完整一笔订单从下单到签收的 7 步流程：

```
步骤  代理商                    服务端                         Bitable
────  ──────                   ──────                         ───────
 ①   order.html 提交订单 ──→  POST /api/submit
                                 ├→ 生成订单号 ORD-XXXX
                                 ├→ 写订单主表 (一单一条，状态=待处理)
                                 ├→ 写镜片明细表 (一眼一条，生成16位HEX镜片码)
                                 ├→ QR图片 → public/qrcodes/
                                 └→ 库存如有货则扣减

 ②   工厂确认 ─────────────→  POST /api/order/:no/confirm
                                 ├→ 镜片码如已生成则跳过（幂等）
                                 └→ 状态：待处理 → 生产中

 ③   CLI 合单发货 ──────────→  logistics.js ship-batch
                                 ├→ 按代理商ID分组，共用快递单号
                                 ├→ 写入物流公司/快递单号/发货时间
                                 ├→ 状态：生产中 → 已发货
                                 └→ 飞书发货卡片

 ④   CLI 生成通行单 ────────→  logistics.js slip-batch
                                 ├→ 读订单主表(物流信息)
                                 ├→ 读镜片明细表(处方+镜片码)
                                 └→ 生成A4 HTML随货通行单

 ⑤   代理商下载工厂包 ─────→  GET /api/order/:no/factory-zip
                                 ├→ 镜片明细表 → Excel(每行一眼)
                                 ├→ 镜片码 → 标签HTML+QR
                                 └→ 打包ZIP下载

 ⑥   消费者签收 ───────────→  logistics.js deliver --order ORD-xxx
                                 ├→ 状态：已发货 → 已签收
                                 └→ 飞书签收卡片

 ⑦   消费者扫码验真 ────────→  GET /verify/:lensCode
                                 ├→ 镜片码 → 镜片明细表
                                 ├→ 订单编号 → 订单主表
                                 └→ 显示验真结果+处方信息
```

**状态流转：** `待处理 → 生产中 → 已发货 → 已签收`

---

## 二、系统全景

```
代理商浏览器                    服务端 (server.js)               飞书 Bitable
─────────────                ──────────────────              ────────────
  order.html ──POST /api/submit──→ 写入订单主表 (一单一条)
                                   写入镜片明细表 (一眼一条)
                                   生成镜片码 + QR
                                   扣减库存(如有货)

  track.html ──GET /api/orders───→ 读取订单主表 ──→ 返回订单列表
                GET /api/order/:no  读取镜片明细表 ──→ 返回处方数据

logistics.js (CLI)              飞书 IM 通知
───────────────                ──────────
  ship / ship-batch ──更新──→ 订单主表(状态→已发货) ──→ 发货卡片
  deliver           ──更新──→ 订单主表(状态→已签收) ──→ 签收卡片
  slip / slip-batch ──读取──→ 订单主表 + 镜片明细表 ──→ 生成HTML通行单
```

---

## 二、数据模型

### 两张核心表

**订单主表** (`tblk9Ch4gk2uQ1zG`) — 一单一条

| 字段 | 类型 | 说明 |
|------|------|------|
| 订单编号 | 文本(主键) | `ORD-YYYYMMDD-XXXXXX`，下单时自动生成 |
| 终端客户 | 文本 | 终端客户机构名称 |
| 客户ID | 文本 | 终端客户在Bitable中的ID |
| 下单日期 | 日期时间 | 下单时间戳 |
| 顾客姓名 | 文本 | 配镜顾客 |
| 产品型号 | 文本 | SKU名称 |
| 数量 | 数字 | 镜片数量（片） |
| 是否装配 | 单选 | "是" / "否" |
| 代理商名称 | 单选 | 代理商全称 |
| 代理商ID | 文本 | `AG-XXX` |
| 备注 | 文本 | 每个顾客的特殊要求 |
| 收货地址 | 文本 | 终端客户收货地址 |
| 联系人 | 文本 | 收货联系人 |
| 联系电话 | 电话 | 联系电话 |
| 镜片码 | 文本 | 该订单的镜片码汇总 |
| 订单状态 | 单选 | 待处理→生产中→已发货→已签收 |
| 预计交期 | 日期时间 | 系统估算的交货日期 |
| 订单来源 | 单选 | "代理商门户" |
| 物流公司 | 文本 | 顺丰/中通/韵达 |
| 快递单号 | 文本 | 物流单号 |
| 发货时间 | 日期时间 | 发货时间戳 |
| 签收时间 | 日期时间 | 签收时间戳 |
| 物流状态 | 单选 | 待处理/已发货/运输中/已签收 |

**镜片明细表** (`tblC7pve7ObFgIOl`) — 一眼一条

| 字段 | 类型 | 说明 |
|------|------|------|
| 镜片码(唯一) | 文本 | 16位HEX，唯一标识 |
| 订单编号 | 文本 | 关联订单主表的订单编号 |
| 镜片码 | 文本 | 与"镜片码(唯一)"相同 |
| 顾客姓名 | 文本 | 配镜顾客 |
| 产品型号 | 文本 | SKU |
| 眼别 | 单选 | "右眼" / "左眼" |
| 球镜SPH | 数字 | 处方球镜 |
| 柱镜CYL | 数字 | 处方柱镜 |
| 轴位AXIS | 数字 | 处方轴位 |
| 是否装配 | 单选 | "是" / "否" |
| 代理商名称 | 文本 | |
| 代理商ID | 文本 | |
| 订单状态 | 单选 | 与主表同步 |

### 辅助表（只读引用）

| 表 | Table ID | 用途 |
|----|----------|------|
| 代理商表 | `tblHsgGbJWkB31qu` | 代理商认证（ID、名称、Token） |
| 终端客户表 | `tbltXNNhF65EBl17` | CRM同步的客户数据 |
| SKU主数据 | `tblwQsvGAahoeoJV` | 产品目录+库存 |
| 成品库存 | `tblUF49B6i53MV2O` | 库存数量 |

---

## 三、模块划分

### 3.1 下单模块 (`server.js` — POST /api/submit + POST /api/order/:no/confirm)

**下单流程：**
```
前端表单(order.html)
  ↓ 提交 { token, terminalCustomer, patients[] }
  ↓
验证代理商token + 必填字段
  ↓
生成订单号 ORD-YYYYMMDD-XXXXXX
  ↓
┌─ 订单主表：每个patient写1行
│  字段：订单编号、顾客姓名、产品型号、数量、是否装配、
│        终端客户、联系人、联系电话、收货地址、代理商、备注
│
├─ 镜片明细表：每个patient的每只眼写1行
│  字段：订单编号、顾客姓名、产品型号、眼别、SPH、CYL、AXIS、
│        是否装配、代理商
│
├─ 生成镜片码：每片16位HEX → 写入镜片明细表
│  → QR码图片保存到 public/qrcodes/
│
└─ 库存扣减：如有货，扣减成品库存表
```

**确认流程（POST /api/order/:no/confirm）：**
```
工厂确认订单
  ↓
检查镜片码是否已生成（下单时已生成则跳过，幂等）
  ↓
状态：待处理 → 生产中
```

**关联：** 读取代理商表(认证)、SKU表(校验)、库存表(扣减)

**流程：**
```
前端表单(order.html)
  ↓ 提交 { token, terminalCustomer, patients[] }
  ↓
验证代理商token + 必填字段
  ↓
生成订单号 ORD-YYYYMMDD-XXXXXX
  ↓
┌─ 订单主表：每个patient写1行
│  字段：订单编号、顾客姓名、产品型号、数量、是否装配、
│        终端客户、联系人、联系电话、收货地址、代理商、备注
│
├─ 镜片明细表：每个patient的每只眼写1行
│  字段：订单编号、顾客姓名、产品型号、眼别、SPH、CYL、AXIS、
│        是否装配、代理商
│
├─ 生成镜片码：每片16位HEX → 写入镜片明细表
│  → QR码图片保存到 public/qrcodes/
│
└─ 库存扣减：如有货，扣减成品库存表
```

**关联：** 读取代理商表(认证)、SKU表(校验)、库存表(扣减)

---

### 3.2 查询模块 (`server.js` — GET /api/orders, /api/order/:no)

**两个端点：**
- `/api/orders?t=token` — 订单列表（从订单主表读，按状态/日期筛选）
- `/api/order/:no?t=token` — 订单详情（主表+镜片明细表联合）

**数据流：**
```
订单列表 → 只读订单主表（不含处方数据）
订单详情 → 读订单主表 + 查镜片明细表(按订单编号关联) → 返回处方+镜片码
```

**关联：** 通过 `订单编号` 字段关联两张表

---

### 3.3 验真模块 (`server.js` — GET /verify/:lensCode)

**公开访问，无需登录。**

```
消费者扫码 → GET /verify/ABCDEF1234567890
  ↓
在镜片明细表中查找镜片码
  ↓
找到 → 通过订单编号查订单主表获取基本信息
  ↓
从镜片明细表获取眼别/SPH/CYL → 渲染验真页面
```

**关联：** 镜片码 → 镜片明细表 → 订单编号 → 订单主表

---

### 3.4 物流模块 (`logistics.js` — CLI工具)

**状态流转：** `待处理 → 生产中 → 已发货 → 已签收`

| 命令 | 读 | 写 | 通知 |
|------|----|----|------|
| `ship` | 主表(生产中) | 主表→已发货 | 飞书发货卡 |
| `ship-batch` | 主表(生产中) | 主表→已发货(合单) | 飞书合单卡 |
| `deliver --order` | 主表(按订单编号) | 主表→已签收 | 飞书签收卡 |
| `slip --order` | 主表+明细表 | — | 生成HTML |
| `slip-batch` | 主表+明细表 | — | 生成HTML |
| `status` | 主表(有快递单号) | — | 控制台输出 |
| `webhook` | 主表 | 主表→已签收 | 飞书签收卡 |

**合单逻辑：** 同一代理商的多笔订单共用一个快递单号，一张通行单

**处方数据来源：** 随货通行单/标签从镜片明细表读眼别/SPH/CYL/AXIS，不从主表读

**关联：** 按 `订单编号` 从主表找到物流信息，再用同一编号查明细表获取处方

---

### 3.5 工厂导出模块 (`server.js` — GET /api/order/:no/factory-zip)

```
按订单编号查镜片明细表
  ↓
生成 Excel (每行一眼，含SPH/CYL/AXIS/镜片码)
  ↓
生成标签 HTML (镜片码+QR+处方)
  ↓
打包 ZIP 下载
```

**关联：** 只读镜片明细表

---

### 3.6 通知模块 (`server.js` webhook + `logistics.js` notify)

- **下单通知：** POST /api/submit 成功后，通过飞书Webhook发卡片
- **发货通知：** ship/ship-batch 后，通过飞书IM API发私信卡片
- **签收通知：** deliver/webhook 后，发绿色签收卡片

---

### 3.7 Excel 解析模块 (`server.js` — POST /api/excel-parse)

**用途：** 代理商上传医院处方 Excel → AI 自动提取患者+处方 → 填入下单表单

```
代理商上传 Excel (.xlsx/.xls)
  ↓
XLSX 解析 → 原始行数据（前50行）
  ↓
MiMo 大模型 (mimo-v2-pro) → 提取结构化处方 JSON
  ↓
后处理：SKU 校验 + 数值归一化（SPH 0.25步长、AXIS 0-180）
  ↓
返回 patients[] + warnings[]
  ↓
前端弹窗预览 → "导入表单" → 自动填充患者卡片
```

**技术栈：**
- LLM：小米 MiMo mimo-v2-pro（OpenAI 兼容格式，`ENV.MIMO_API_URL` + `ENV.MIMO_API_KEY`）
- Excel 解析：已安装的 `xlsx` 库
- 文件传输：Base64 编码在 JSON body 中（无 multipart，零新依赖）

**关联：** 读取 SKU 表(`getSkusWithInventory` + `getModelSkus`) 用于 SKU 校验

---

## 四、模块间关联图

```
              Excel 上传                    ┌─────────────┐
                ↓                           │  订单主表    │ ← 物流模块写入(状态)
          Excel解析模块 ──导入表单──→ 下单模块│ (一单一条)   │ ← 下单模块写入
          (MiMo mimo-v2-pro)               └──────┬──────┘
                                                   │ 订单编号(关联)
                                            ┌──────┴──────┐
                                            │  镜片明细表  │ ← 下单模块写入
                                            │ (一眼一条)   │ ← 生成镜片码+QR
                                            └──────┬──────┘
                                                   │ 镜片码
                                      ┌────────────┼────────────┐
                                      ↓            ↓            ↓
                                 验真模块     工厂导出      通行单/标签
                                 (扫码查真)   (Excel+QR)   (logistics.js)
```

**核心关联字段：**
- 订单主表 ↔ 镜片明细表：通过 `订单编号`
- 镜片明细表 → 消费者：通过 `镜片码`(16位HEX + QR)

---

## 五、CLI 工具清单

```bash
# 启动门户
node server.js                    # 端口 3210

# 物流操作
node logistics.js ship            # 逐单发货
node logistics.js ship-batch      # 合单发货(按代理商合并)
node logistics.js deliver --order ORD-xxx  # 签收
node logistics.js slip --order ORD-xxx     # 单订单通行单
node logistics.js slip-batch      # 合单通行单
node logistics.js status          # 物流汇总
node logistics.js webhook         # 快递回调服务(端口 3211)

# 迁移/初始化
node migrate_split_tables.js      # 镜片明细表创建
node migrate_tables.js            # Bitable字段迁移
node setup_tables.js              # 表结构初始化
node import_real_skus.js          # 导入SKU数据
```

---

## 六、环境依赖

```
shared/.env          飞书应用凭证
node_modules/        qrcode, xlsx, http(内置)
```

两表共享同一个飞书多维表格 App Token (`B3xQbbqicaome1sKdZbcwdk8nWg`)，通过 `FEISHU_APP_ID` + `FEISHU_APP_SECRET` 获取 tenant_access_token。
