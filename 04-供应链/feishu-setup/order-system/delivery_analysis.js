...
function diagnoseRootCauses(skuGaps, molds, blanks, factories) {
  ...
  for (const r of molds) {
    ...
    moldMap[f["产品型号"]] = { remaining, id: f["模芯编号"], total: f["总寿命（次）"] || 0 };
  }

  // Blank inventory index
  const blankMap = {};
  for (const r of blanks) {
    blankMap[r.fields["产品型号"]] = r.fields["当前毛坯库存"] || 0;
  }
...
async function main() {
  ...
  if (problemSKUs.length > 0) {
    ...
    log(`  ${"产品型号".padEnd(28)} ${"ABC".padEnd(4)} ${"Orders".padEnd(8)} ${"Fill%".padEnd(8)} ${"Stock".padEnd(8)} ${"Cover(wk)".padEnd(10)}`);
    log(`  ${"─".repeat(66)}`);
