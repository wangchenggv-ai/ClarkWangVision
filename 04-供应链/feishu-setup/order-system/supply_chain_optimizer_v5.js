...
try {
  ...
  // 绕过init，直接修改feishu.js内部变量（通过全局process.env已经生效）
  // feishu.js的getFeishuToken()使用ENV.FEISHU_APP_ID，但我们已经通过loadEnv()设置了process.env
  // 只需要设置BASE和APP_TOKEN
  if (feishuModule.init) {
    feishuModule.init({
      base: 'https://open.feishu.cn',
      appToken: 'B3xQbbqicaome1sKdZbcwdk8nWg',
      env: process.env  // 这个会被feishu.js赋值给内部的ENV变量
    });
  }
...
async function fetchHistoricalOrders(days) {
  if (!initialized || !listRecords || !TABLES) {
