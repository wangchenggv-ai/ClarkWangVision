/**
 * remove_40_orders.js — Remove 40 test orders to reduce total count
 *
 * Usage: node remove_40_orders.js
 */
...
async function main() {
  ...
  // Filter test orders (TEST- prefix)
  ...
  if (testOrders.length === 0) {
    console.log(`⚠️  No test orders found to remove.`);
    return;
  }

  // Take all test orders to delete
  const ordersToDelete = testOrders;
  console.log(`🗑️  Deleting ${ordersToDelete.length} test orders...\n`);

  let deletedCount = 0;
  for (const order of ordersToDelete) {
    const recordId = order.record_id;
    const orderNo = order.fields["订单编号"];
    const sku = order.fields["产品型号"];
  ...
  console.log(`\n✅ Done. ${deletedCount}/40 orders deleted.`);
