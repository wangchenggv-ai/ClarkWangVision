...
async function main() {
  ...
  let delivActualFill = 0, delivProcessed = 0;
  let delivOverdue = 0, delivOnTime = 0, delivPending = 0;
  const delivSkuStats = {};
  for (const r of orders) {
    const f = r.fields;
    const sku = f["产品型号"] || "";
    if (!delivSkuStats[sku]) delivSkuStats[sku] = { total: 0, totalQty: 0, overdue: 0 };
  ...
  delivProcessed = delivOnTime + delivOverdue + delivPending;
  delivActualFill = delivProcessed > 0 ? (delivOnTime / delivProcessed * 100) : 0;
  ...
  const invMapDeliv = {};
  for (const r of inventory) invMapDeliv[r.fields["产品型号"]] = r.fields;
  const demandMap = {};
  ...
  <div style="background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #e8eaef; box-shadow: 0 1px 4px rgba(0,0,0,0.04);">
    ...
    <div class="kpi-row" style="padding:0;">
      <div class="kpi ${delivActualFill >= 80 ? 'green' : delivActualFill >= 50 ? 'orange' : 'red'}" style="background:#fff">
        ...
        <div class="label">准时${delivOnTime} / 总${delivProcessed}单</div>
      </div>
